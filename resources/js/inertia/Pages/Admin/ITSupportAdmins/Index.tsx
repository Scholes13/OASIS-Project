...
import { Search, Headphones, Building2, Users } from 'lucide-react';
...
export default function Index({ assignments, businessUnits, adminCounts, reportAccessCounts, filters }: Props) {
    ...
    const applyFilters = (overrides: Record<string, string> = {}) => {
        router.get(route('admin.it-support-admins.index'), {
    ...
    const handleToggle = (id: number) => {
        setTogglingId(id);
        router.post(route('admin.it-support-admins.toggle', { id }), {}, {
            preserveScroll: true,
    ...
    const handleToggleReport = (id: number) => {
        ...
        router.post(route('admin.it-support-admins.toggle-report', { id }), {}, {
            preserveScroll: true,
    ...
    return (
        <>
            <Head title="IT Support Assignment" />

            <div className="p-6 space-y-6">
                {/* Header */}
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">IT Support Assignment</h1>
                    <p className="mt-1 text-sm text-gray-600">
                        Assign users sebagai IT Support Admin per Business Unit. IT Support Admin dapat mengelola ticket dan melihat consolidated report.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4">
                        <div className="p-3 bg-cyan-50 rounded-xl">
                            <Headphones className="w-6 h-6 text-cyan-600" strokeWidth={1.5} />
                        </div>
                        <div>
                            <p className="text-sm text-gray-500">Total IT Support Admins</p>
                            <p className="text-xl font-bold text-gray-900">{totalAdmins}</p>
                        </div>
                    </div>
                    <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4">
                        <div className="p-3 bg-teal-100 rounded-xl">
                            <Headphones className="w-6 h-6 text-teal-600" strokeWidth={1.5} />
                        </div>
                        <div>
                    ...
                    {businessUnits.slice(0, 2).map((bu) => (
                        <div key={bu.id} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4">
                            <div className="p-3 bg-cyan-50 rounded-xl">
                                <Building2 className="w-6 h-6 text-cyan-600" strokeWidth={1.5} />
                            </div>
                            <div>
                ...
                <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                {['User', 'Business Unit', 'Department', 'Position', 'IT Support Admin', 'Report Access'].map((h) => (
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {assignments.data.length === 0 ? (
                                ...
                                <tr key={a.id} className={cn('hover:bg-gray-50 transition-colors', a.is_it_support_admin && 'bg-cyan-50')}>
                                    <td className="px-4 py-3">
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 bg-cyan-600 rounded-full flex items-center justify-center text-xs font-medium text-white">
                                    ...
                                    <td className="px-4 py-3">
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                a.is_it_support_admin ? 'bg-cyan-600' : 'bg-gray-200',
                                        ...
                                        >
                                            <span
                                                className={cn(
                                                    ...
                                                    a.is_it_support_admin ? 'translate-x-6' : 'translate-x-1'
                                                )}
                                            />
                                        </button>
                                    </td>
                                    <td className="px-4 py-3">
                                        {a.is_it_support_admin ? (
                                            <button
                                                ...
                                                className={cn(
                                                    ...
                                                    a.is_it_support_report_access ? 'bg-teal-600' : 'bg-gray-200',
                                            ...
                                            >
                                                <span
                                                    className={cn(
                                                        'inline-block h-4 w-4 transform rounded-full bg-white transition-transform',
                                                        a.is_it_support_report_access ? 'translate-x-6' : 'translate-x-1'
                                                    )}
                                                />
                                            </button>
                                        ) : (
                                            <div className="relative group">
                                                ...
                                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 px-2 py-1 bg-gray-800 text-white text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                                                    Aktifkan IT Support Admin terlebih dahulu
                                                </div>
                                            </div>
                                        )}
                                    </td>
                                ...
                                {assignments.meta.links.map((link, i) => (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            link.active ? 'bg-cyan-600 text-white' : 'text-gray-600 hover:bg-gray-100',
