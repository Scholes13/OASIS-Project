...
import { purchasingTaskCalendarStyles } from './purchasingTaskCalendarStyles';
...
function EventContent({ event, view }: { event: any; view: string }) {
    ...
    const sourceDepartment = task.taskable?.department?.name || task.department?.name;
    ...
    if (isMonthView) {
        return (
            ...
            >
                ...
                />
                <span className="min-w-0 truncate" style={{ color: status.text }}>
                    <span className="font-semibold">{number || 'Task'}</span>
                    {sourceDepartment && <span className="ml-1 opacity-75">· {sourceDepartment}</span>}
                </span>
            </div>
        );
    }
...
export function PurchasingTaskCalendar({ tasks, onTaskClick }: PurchasingTaskCalendarProps) {
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <style>{purchasingTaskCalendarStyles}</style>
        </div>
    );
}
