<x-app-layout>
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex items-center space-x-3">
                                                ...
                                                @if($bu->code !== 'WG')
                                                    <button type="button" 
                                                            onclick="openDeleteModal('{{ $bu->id }}', '{{ $bu->name }}')"
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            @elseif (session('business_unit_created'))
                @php $data = session('business_unit_created'); @endphp
                showBusinessUnitToast("{{ $data['title'] }}", "{{ $data['name'] }}", 'success');
            @elseif (session('success_create_unit'))
                @php $data = session('success_create_unit'); @endphp
                showBusinessUnitToast("{{ $data['title'] }}", "{{ $data['name'] }}", 'success');
            @elseif (session('success_update_unit'))
                @php $data = session('success_update_unit'); @endphp
                showBusinessUnitToast("{{ $data['title'] }}", "{{ $data['name'] }}", 'success');
            @elseif (session('success_delete_unit'))
                @php $data = session('success_delete_unit'); @endphp
                showBusinessUnitToast("{{ $data['title'] }}", "{{ $data['name'] }}", 'success');
