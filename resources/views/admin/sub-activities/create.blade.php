<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Create Sub-Activity</h1>
                <p class="text-sm text-gray-600 mt-1">Add a new sub-activity for task categorization</p>
            </div>
            <a href="{{ route('admin.sub-activities.index', request()->only('activity_type')) }}" 
    ...
    <div class="max-w-2xl">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <form action="{{ route('admin.sub-activities.store') }}" method="POST" class="p-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <select name="activity_type_id" 
                        ...
                        @foreach($activityTypes as $type)
                            <option value="{{ $type->id }}" {{ old('activity_type_id', $selectedActivityTypeId) == $type->id ? 'selected' : '' }}>
                                {{ $type->name }} ({{ $type->code }})
                            </option>
                        @endforeach
                    </select>
