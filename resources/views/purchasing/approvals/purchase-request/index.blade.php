        ...
        <div class="border-b border-gray-200 px-6 py-4">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <button type="button" onclick="window.location.reload()" class="inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition-colors">
        ...
        <div class="border-b border-gray-200 px-6 py-8">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <a href="{{ route('approvals.index', ['filter' => 'pending']) }}" 
                   class="relative flex items-center px-6 py-5 rounded-lg border border-gray-200 transition-all duration-200 hover:shadow-md hover:border-primary hover:bg-blue-600 cursor-pointer {{ request('filter') === 'pending' ? 'bg-primary ring-2 ring-primary border-primary' : 'bg-white' }}">
                    ...
                    <div class="w-1 h-16 bg-primary rounded-full" style="margin-right: 1rem;"></div>
                    <div>
                        <p class="text-3xl font-semibold text-primary">{{ $pendingApprovals->count() }}</p>
                ...
                <a href="{{ route('approvals.index', ['filter' => 'approved']) }}" 
                   class="relative flex items-center px-6 py-5 rounded-lg border border-gray-200 transition-all duration-200 hover:shadow-md hover:border-primary hover:bg-blue-600 cursor-pointer {{ request('filter') === 'approved' ? 'bg-primary ring-2 ring-primary border-primary' : 'bg-white' }}">
                    <svg class="absolute top-4 right-4 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path>
                    </svg>
                    <div class="w-1 h-16 bg-primary rounded-full" style="margin-right: 1rem;"></div>
                    <div>
                        <p class="text-3xl font-semibold text-primary">{{ $approvalStats['total_approved'] ?? 0 }}</p>
                ...
                <a href="{{ route('approvals.index', ['filter' => 'rejected']) }}" 
                   class="relative flex items-center px-6 py-5 rounded-lg border border-gray-200 transition-all duration-200 hover:shadow-md hover:border-primary hover:bg-blue-600 cursor-pointer {{ request('filter') === 'rejected' ? 'bg-primary ring-2 ring-primary border-primary' : 'bg-white' }}">
                    <svg class="absolute top-4 right-4 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path>
                    </svg>
                    <div class="w-1 h-16 bg-primary rounded-full" style="margin-right: 1rem;"></div>
                    <div>
                        <p class="text-3xl font-semibold text-primary">{{ $approvalStats['total_rejected'] ?? 0 }}</p>
                ...
                <a href="{{ route('approvals.index') }}" 
                   class="relative flex items-center px-6 py-5 rounded-lg border border-gray-200 transition-all duration-200 hover:shadow-md hover:border-primary hover:bg-blue-600 cursor-pointer bg-white">
                    <svg class="absolute top-4 right-4 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path>
                    </svg>
                    <div class="w-1 h-16 bg-primary rounded-full" style="margin-right: 1rem;"></div>
                    <div>
                        <p class="text-3xl font-semibold text-primary">{{ $pendingApprovals->count() + ($approvalStats['total_approved'] ?? 0) + ($approvalStats['total_rejected'] ?? 0) }}</p>
            ...
            @if($filteredApprovals->isEmpty())
                <div class="py-12 text-center">
                    ...
                    <p class="text-sm text-gray-500">
                        @if($filter)
                            No {{ $filter }} approvals found. <a href="{{ route('approvals.index') }}" class="text-primary hover:text-primary font-medium">Clear filter</a>
                        @else
