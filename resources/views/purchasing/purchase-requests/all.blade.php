    ...
    <div class="w-full">
        ...
        <div class="border-b border-gray-200 px-6 py-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500">Manage all purchase requests in {{ session('current_business_unit_name') }}</p>
                </div>
        ...
        <div class="border-b border-gray-200 px-6 py-8">
            <div style="display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 1.5rem;">
                ...
                <!-- {{ $category->name }} -->
                <a href="{{ route('purchase-requests.all', ['category' => $category->id]) }}" 
                   class="relative flex items-center px-6 py-5 rounded-lg border border-gray-200 transition-all duration-200 hover:shadow-md hover:border-indigo-300 hover:bg-indigo-50 cursor-pointer {{ $categoryFilter == $category->id ? 'bg-indigo-50 ring-2 ring-indigo-500 border-indigo-500' : 'bg-white' }}">
                    ...
                    <div>
                        <p class="text-3xl font-semibold text-indigo-600">{{ $categoryStats[$category->id] ?? 0 }}</p>
                ...
                <!-- Total Documents -->
                <a href="{{ route('purchase-requests.all', ['all' => 1]) }}" 
                   class="relative flex items-center px-6 py-5 rounded-lg border border-gray-200 transition-all duration-200 hover:shadow-md hover:border-indigo-300 hover:bg-indigo-50 cursor-pointer {{ ($showAll && !$categoryFilter) ? 'bg-indigo-50 ring-2 ring-indigo-500 border-indigo-500' : 'bg-white' }}">
                    <svg class="absolute top-4 right-4 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path>
                    </svg>
                    <div class="w-1 h-16 bg-indigo-600 rounded-full" style="margin-right: 1rem;"></div>
        ...
        <div class="px-6 py-4">
            @if($purchaseRequests->count() > 0)
                <table class="min-w-full">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        @endphp
                        <tr class="hover:bg-gray-50 transition-colors cursor-pointer" onclick="window.location='{{ route('purchase-requests.show', $pr) }}'">
                            <td class="px-3 py-4">
                    ...
                    <div class="px-3 py-4 border-t border-gray-200">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                @else
                                    <a href="{{ $purchaseRequests->previousPageUrl() }}" class="px-3 py-1 text-sm text-gray-700 hover:bg-gray-100 rounded">&lt;</a>
                                @endif
                                ...
                                @foreach($purchaseRequests->getUrlRange(1, $purchaseRequests->lastPage()) as $page => $url)
                                    ...
                                    @else
                                        <a href="{{ $url }}" class="px-3 py-1 text-sm text-gray-700 hover:bg-gray-100 rounded">{{ $page }}</a>
                                    @endif
                                @endforeach

                                @if($purchaseRequests->hasMorePages())
                                    <a href="{{ $purchaseRequests->nextPageUrl() }}" class="px-3 py-1 text-sm text-gray-700 hover:bg-gray-100 rounded">&gt;</a>
                                @else
                                    <span class="px-3 py-1 text-sm text-gray-400 cursor-not-allowed">&gt;</span>
                                @endif
                            </div>
            ...
            @else
                ...
                <div class="py-12 text-center">
                    ...
                    <p class="text-sm text-gray-500">No purchase requests have been created in this business unit yet.</p>
                </div>
            @endif
        </div>
    </div>
...
{{-- Listen for Business Unit switch and reload page --}}
<script>
