...
<head>
    ...
    <title>PR Verification - {{ $stockRequest->st_number }}</title>
            ...
            <div class="bg-white border border-gray-200 mb-4">
                <div class="px-4 py-3 border-b border-gray-200 bg-gray-50">
                    <p class="text-xs font-medium text-gray-900">
                        PR: <span class="font-bold">{{ $stockRequest->st_number }}</span>
                    </p>
                </div>
                ...
                <div class="px-4 py-3">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                        <div>
                            ...
                            <p class="text-xs text-gray-900">{{ $stockRequest->user->name }}</p>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-500 mb-0.5">Department</label>
                            <p class="text-xs text-gray-900">{{ $stockRequest->department->name }}</p>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-500 mb-0.5">Business Unit</label>
                            <p class="text-xs text-gray-900">{{ $stockRequest->businessUnit->name }}</p>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-500 mb-0.5">Request Date</label>
                            <p class="text-xs text-gray-900">{{ \Carbon\Carbon::parse($stockRequest->date_of_request)->format('d/m/Y') }}</p>
                        </div>
                    </div>
                    
                    <div class="border-t border-gray-200 pt-3">
                        <label class="block text-xs font-medium text-gray-500 mb-1">Purpose / Used For</label>
                        <p class="text-xs text-gray-900 leading-relaxed">{{ $stockRequest->used_for }}</p>
                    </div>
                </div>
            </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tfoot class="bg-gray-50">
                            <tr>
                                ...
                                <td class="px-3 py-2 text-xs font-bold text-gray-900">{{ $stockRequest->currency }} {{ number_format($stockRequest->total_amount, 0) }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-center py-2 {{ $verificationData['type'] === 'requestor' ? 'bg-green-50 -mx-2 px-2' : '' }}">
                            <div class="flex-1">
                                ...
                                <div class="text-xs text-gray-500">Requestor • {{ $stockRequest->submitted_at ? $stockRequest->submitted_at->format('d/m/Y H:i') : 'N/A' }}</div>
                            </div>
                            <div>
                        ...
                        <!-- Approval Steps -->
                        @foreach($stockRequest->approvals->sortBy('step_order') as $step)
